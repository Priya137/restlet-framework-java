...
import org.restlet.ext.odata.json.JsonStreamReaderFactory.JsonParseException;
import org.restlet.ext.odata.json.JsonStreamReaderFactory.JsonStreamReader;
import org.restlet.ext.odata.json.JsonStreamReaderFactory.JsonStreamReader.JsonEndPropertyEvent;
import org.restlet.ext.odata.json.JsonStreamReaderFactory.JsonStreamReader.JsonEvent;
import org.restlet.ext.odata.json.JsonStreamReaderFactory.JsonStreamReader.JsonStartPropertyEvent;
import org.restlet.ext.odata.json.JsonStreamReaderFactory.JsonStreamReader.JsonValueEvent;
import org.restlet.ext.odata.json.JsonStreamReaderFactory.JsonStreamTokenizer;
import org.restlet.ext.odata.json.JsonStreamReaderFactory.JsonStreamTokenizer.JsonToken;
import org.restlet.ext.odata.json.JsonStreamReaderFactory.JsonStreamTokenizer.JsonTokenType;

/**
 * The Class JsonStreamReaderFactory contains API for parsing and iterating
 * through json response.
 * 
 */
public class JsonStreamReaderFactory {
	...
	public static class JsonParseException extends RuntimeException {
		...
		private static final long serialVersionUID = 1409832870999L;
	...
	public interface JsonStreamTokenizer {
		...
		public enum JsonTokenType {
			LEFT_CURLY_BRACKET, RIGHT_CURLY_BRACKET, LEFT_BRACKET, RIGHT_BRACKET, COMMA, COLON, TRUE, FALSE, NULL, NUMBER, STRING, NaN;
		}
...
class JsonEventImpl implements JsonEvent {
	...
	public String toString() {
		...
		if (isStartObject()) {
			...
			bld.append("StartProperty(").append(asStartProperty().getName())
					.append(")");
		} else if (isEndProperty()) {
			if (asEndProperty().getValue() == null) {
				bld.append("EndProperty(").append("<null>").append(")");
			} else {
				bld.append("EndProperty(").append(asEndProperty().getValue())
						.append(")");
			}
		} else if (isValue()) {
...
class JsonStartPropertyEventImpl extends JsonEventImpl implements
...
class JsonEndPropertyEventImpl extends JsonEventImpl implements
...
class JsonStreamTokenizerImpl implements JsonStreamTokenizer {
			...
			if (state == TokenizerState.DEFAULT) {
				...
				if (quote) {
					if ('b' == c) {
						...
						buffer.append((char) Integer.parseInt(new String(
								next(4)), 16));
					} else {
						throw new JsonParseException(
								"illegal escaped character " + c);
					}
					quote = false;
				} else if ('\\' == c) {
					quote = true;
				} else if ('"' == c) {
					token = new JsonToken(JsonTokenType.STRING,
							buffer.toString());
				} else {
					buffer.append(c);
					quote = false;
				...
				if ('-' == c || Character.isDigit(c) || 'E' == c || 'e' == c
						|| '+' == c || '.' == c) {
					...
					token = new JsonToken(JsonTokenType.NUMBER,
							buffer.toString());
				}
			}
		}
	...
	private boolean hasConstantParsed(StringBuilder memory) {
		if (isTrue(memory)) {
			...
			token = new JsonToken(JsonTokenType.NaN, "NaN");
		}
		return token != null;
	}
	...
	private boolean isTrue(StringBuilder str) {
		return str.length() == 4 && str.charAt(0) == 't'
				&& str.charAt(1) == 'r' && str.charAt(2) == 'u'
				&& str.charAt(3) == 'e';
	}

	private boolean isFalse(StringBuilder str) {
		return str.length() == 5 && str.charAt(0) == 'f'
				&& str.charAt(1) == 'a' && str.charAt(2) == 'l'
				&& str.charAt(3) == 's' && str.charAt(4) == 'e';
	}

	private boolean isNull(StringBuilder str) {
		return str.length() == 4 && str.charAt(0) == 'n'
				&& str.charAt(1) == 'u' && str.charAt(2) == 'l'
				&& str.charAt(3) == 'l';
	}

	private boolean isNaN(StringBuilder str) {
		return str.length() == 3 && str.charAt(0) == 'N'
				&& str.charAt(1) == 'a' && str.charAt(2) == 'N';
	}
...
class JsonStreamReaderImpl implements JsonStreamReader {
		...
		if (hasNext()) {
			...
			case NONE:
				if (token.type != JsonTokenType.LEFT_CURLY_BRACKET) {
					throw new JsonParseException(
							"no JSON format must start with {");
				} else {
			...
			case OBJECT:
				if (expectCommaOrEnd) {
					if (token.type == JsonTokenType.COMMA) {
						if (!tokenizer.hasNext()) {
							throw new JsonParseException(
									"no JSON format premature end");
						}
						token = tokenizer.nextToken();
					} else if (token.type != JsonTokenType.RIGHT_CURLY_BRACKET) {
						throw new JsonParseException(
								"no JSON format expected , or ] got "
										+ token.type);
					}
				...
				case STRING:
					if (!tokenizer.hasNext()
							|| tokenizer.nextToken().type != JsonTokenType.COLON) {
						throw new JsonParseException(
								"no JSON format : expected afer " + token.value);
					}
	...
	private JsonEvent createEndPropertyEvent(final String value,
			final JsonTokenType valueTokenType) {
		state.pop();
	...
	public void skipNestedEvents() {
		if (!previousEvent.isStartProperty() && !previousEvent.isStartObject()
				&& !previousEvent.isStartArray())
			return;

		// skip until stack element pushed by a start event has been removed by
